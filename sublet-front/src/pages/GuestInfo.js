...
function User({ user }) {
      ...
      <div className="ml-4 mt-4">
        <div className="w-2/6">
          ...
          <div>
            ...
            <s.SvgHoverButton onClick={setEmailPopUpState} className="justify-end">
              <StyleComponent
                content="FixInfo"
              />
            </s.SvgHoverButton>
            {!user.verify_email &&
              (
                <div>
                  <s.NormalButton className="float-left" onClick={onVerifyEmailHandle}>
                    인증하기
                  </s.NormalButton>
                </div>
              )
  ...
  const userBaseComponent = (
    <div>
      <s.ImageUploadButton onClick={setImagePopUpState} className="object-cover w-46 h-26">
      ...
      <s.NormalText >재학증 {user.verify_school ? '인증 완료✅' : '인증 안됨❌'}</s.NormalText>
