...
export default function Booking(user_id) {
	...
	const { temp_start_day, temp_end_day, day_pay, total_pay, post_key } =
		...
		<div className="ml-4 w-4/5 items-center">
			<div>
				...
				<p className="text-xl font-semibold ml-2">
					{temp_start_day} ~ {temp_end_day} ({total_day}일)
				</p>
				<div>
					{total_day >= 28 && (
					...
					<s.JustifyBlock className="ml-2 font-bold flex justify-between">
						...
						<p className="text-l text-[#2478F6]">
							{priceToString(total_pay)} 원
						</p>
					</s.JustifyBlock>
				</div>
			</div>
			...
			<div>
				...
				<div className="mt-2 ml-4">
					<s.NormalText className="mt-2">
						...
						{part_refund_date} 전에 취소하면 부분 환불을 받으실 수 있습니다.
						<br />
						<br />
						...
						적용되는 기본 규칙, 에어비앤비 재예약 및 환불 정책에 동의하며,
						피해에 대한 책임이 본인에게 있을 경우 에어비앤비가 결제 수단으로
						청구의 조치를 취할 수 있다는 사실에 동의하는 것입니다.
					</s.NormalText>
				</div>
				<div className="mt-4" fullWidth>
					{checkState ? (
						<s.NormalButton onClick={handlePostReservation}>
							결제하기
						</s.NormalButton>
					) : (
