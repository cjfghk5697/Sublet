...
function FetchDeleteReservation(keyNum) {
...
function FetchReservationPost(userID, postKey, startDay, endDay, pay) {
	fetch(`${process.env.REACT_APP_BACKEND_URL}/reservation`, {
		...headerOptions('POST'),
		...bodyData({
			user_id: userID,
...
function FetchGetRequestByRequestId(idList) {
	const [requestInfo, setRequestInfo] = useState([]);
